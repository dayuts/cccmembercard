...
class CCCMemberCardFrame(wx.Frame):
    ...
    def OnButton_load_member_list(self, event):
        ...
        if not os.path.exists('credential.json'):
            wx.MessageBox("credential.json is not available. Please copy example-credential.json to credential.json and modify it")
        else:
            initialization_flag = self.cccmember.initialize_with_credential_json_file('credential.json')
            if ~initialization_flag:
              wx.MessageBox("Unable to initialize connection to wildapricot. Check error log for detailed information")
            else:
              self.gauge_load_status.inc()
